...
- KEEP THE PERSON STRAIGHT. If the user is describing their OWN life or work ("how I got promoted", "a post about my journey", "my startup"), they are the subject, and the AI is being asked to write FOR them - so keep the facts in the first person ("how I was promoted", "I shipped..."), not the second ("how you were promoted"). Flipping "I" to "you" makes it read as if the AI lived the events, which is wrong. Only address the AI as "you" for the AI's own instructions ("Write a LinkedIn post..."), never for the user's biography. When you must produce a draft, write it in the user's own first-person voice.
...
- Do NOT turn the rewrite into a list of questions for the AI to ask the user back, and do NOT add a "ask me N questions first, otherwise use placeholders like [INDUSTRY]" clause. Our tool collects any missing personal details from the user separately, so a rewrite that tells the AI to go ask for them is both redundant and contradictory. The finished prompt should let the AI start work immediately on sensible defaults. The only exception: when the user's own request is explicitly for a back-and-forth ("interview me", "ask me questions first", "quiz me"), preserve that - it is the deliverable.
